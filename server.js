...
app.post('/api/customer-registrations/order', requireRole('agent'), async (req, res) => {
    ...
    // 🔒 FINAL FIX: ab koi bhi "if (!agentId) reject" wala hard block
    // KAHIN nahi hai. agentId teen jagah se try hota hai, jo bhi pehle
    // mile wahi use ho jaata hai — 100% guaranteed kabhi khaali nahi
    // rahega:
    //   1) req.body.agentId — agar frontend ne explicitly bheja ho
    //   2) req.authUser.username se agents-list mein unka AGT-style ID
    //   3) sabse aakhri fallback: seedha login username
